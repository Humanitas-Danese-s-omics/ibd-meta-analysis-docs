...
In TaMMA, you can find the same UMAP decorated in two different ways, where dots represent the different samples. On the left dots are colored by a specific user-selected metadata (see :ref:`color_by_dropdown`), while on the right dots are colored by gene expresion or order/family/species abundance levels (see :ref:`expression_dropdown`).
These two plots can be used to search for specific clusters of samples that share particular metadata (on the left), or expression/abundance level (on the right) at a sample level. Samples can be filtered according to the **Color by** dropdown or by selecting the desired features in the legend below and then by clicking on the **Update plot** button.
...
In the title, it is possible to see how many samples are displayed (n=xxxx), the multidimensional scaling used (human transcriptome, archaeome, bacteriome, eukaryome/mycome, virome) and the metadata/gene/order/family/species used for coloring the dots. 

.. note::

    When a non-human trascriptome multidimensional scaling is selected, samples for which the metatranscriptomics is not available (e.g. samples with just poly-A enrichment as library preparation as an example) will be filtered out, also resulting in changing in the legend where features related only on human transcriptome are filtered out. This have also implication on boxplots where samples will be filtered out too, see :ref:`boxplots`.
...
Under the plots is located a switch entitled "Multiple gene visualization". By activating this switch, a new section will appear below; in this new section you can easily plot many series of box plots of different features depending on the :ref:`expression_dropdown` (human genes if **Human** is selected, or orders/families/species if another value is selected). For performance and usability reasons, it is possible to plot up to 10 elements at a time.
...
Filter comparison by
-----------------------

This dropdown is used to filter the following dropdown (Comparison). It is possible to filter for tissue or fro group (disease) or to not filter at all.
...
This dropdown selects the comparison used for the differential gene expression results and the resulting gene ontology enrichment analysis. Thus, changing this dropdown will update both MA and GO plots, and their relative tables. The comparison follows the scheme of **Tissue Group A vs Tissue Group B**. As mentioned before, the comparisons can be filtered by the precent dropdown.
There are two types of different comparisons:

- Same tissue, different group, filtering by tissue will show only these contrast with the specific tissue selected

- Same group, different tissue, filtering by group will show only these contrast with the specific group selected

It is always possible to search manually by writing inside the dropdown.
...
Below, there is the full metadata table used to populate the :ref:`color_by_dropdown` plus other info like, for example, the GSE link to the specific study on `NCBI GEO <https://www.ncbi.nlm.nih.gov/geo/>`_. The table has native sorting and filtering (by exact match) interactivity options.

Differential analysis
------------------------

This tab have two different sub-tabs:
...
.. note::
    ...
    Similar to :ref:`ma_plot`, clicking on a Gene/Order/Family/Species cell in the table will change the selected element in the right dropdown (see :ref:`gene_species_dropdown`), thus updating all the relevant plots. 
...
Here, we collected a series of previous scientific evidence that is validated in TaMMA and also new evidence found using TaMMA. These two categories are collected in two relative sub-tabs, "Old evidence from the literature confirmed by TaMMA" and "New evidence by TaMMA". The plots here are not related to any of the dropdowns on the top of the website and are not interactive.

The data behind it
======================================

The complete list of raw data analyzed, with the associated metadata, can be fount in the :ref:`summary_and_metadata` tab.
...
Report a bug, suggestions and feature requests
===================================================
...
In the future, as new datasets will be published, we will update TaMMA by adding these as soon as possible. Also, in the future, metagenomic data will be integrated in the platform. 
